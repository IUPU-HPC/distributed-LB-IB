...
int cube2thread_and_task(int BI, int BJ, int BK, GV gv, int *dst_task){
  ...
  // Within a fluid process, each thread operate num of cubes in each dimension
  int cubes_per_thd_x  =  cubes_per_task_x / tx;
  int cubes_per_thd_y  =  cubes_per_task_y / ty;
  int cubes_per_thd_z  =  cubes_per_task_z / tz;
  ...
  i = (BI % cubes_per_task_x) / cubes_per_thd_x;
  j = (BJ % cubes_per_task_y) / cubes_per_thd_y;
  k = (BK % cubes_per_task_z) / cubes_per_thd_z;
...
int global2task_and_thread(int X, int Y, int Z, GV gv, int *dst_task){
  ...
  // rank_y = BJ / cubes_per_task_y;
  // rank_x = BI / cubes_per_task_x;
  *dst_task = (BI / cubes_per_task_x) * num_fluid_task_y * num_fluid_task_z 
        + (BJ / cubes_per_task_y) * num_fluid_task_z 
        + BK / cubes_per_task_z;

  int tx = gv->tx; //number of fluid thread per fluid task along x
  int ty = gv->ty;
  int tz = gv->tz;    

  // Within a fluid process, each thread operate num of blocks in each dimension
  int cubes_per_thd_x = cubes_per_task_x / tx;
  int cubes_per_thd_y = cubes_per_task_y / ty;
  int cubes_per_thd_z = cubes_per_task_z / tz;
